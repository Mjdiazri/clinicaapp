    ...
    private static void cargarPacientes(ClinicaService servicio) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(f))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                servicio.getPacientes().add(new Paciente(Integer.parseInt(p[0].trim()),
    ...
    private static void cargarMedicos(ClinicaService servicio) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(f))) {
            ...
            while ((linea = br.readLine()) != null) {
                if (linea.isBlank()) continue;
                String[] p = linea.split(",", -1);
                servicio.getMedicos().add(new Medico(Integer.parseInt(p[0].trim()), p[1].trim(), p[2].trim(),
    ...
    private static void cargarTurnos(ClinicaService servicio) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(f))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                servicio.getTurnos().add(new Turno(Integer.parseInt(p[0].trim()), pac, med,
                        LocalDateTime.parse(p[4].trim(), FMT), EstadoTurno.valueOf(p[5].trim())));
            }
        } catch (IOException e) {
            System.out.println("Error: " + e.getMessage());
        }
    ...
    private static void guardarPacientes(List<Paciente> lista) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(F_PACIENTES))) {
            for (Paciente p : lista)
                pw.println(p.getId() + "," + p.getCedula() + "," + p.getNombre() + "," + p.getApellido() + ","
                        + p.getTelefono());
        } catch (IOException e) {
